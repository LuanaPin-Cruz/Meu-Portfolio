      ...
      <main>
        <section id="inicio" className="hero section-wrap">
          <div className="hero__copy">
            ...
            <div className="quick-stats" aria-label="Resumo profissional"><div><strong>18</strong><span>anos</span></div><div><strong>3º</strong><span>ano</span></div><div><strong>∞</strong><span>vontade de aprender</span></div></div>
          </div>
        ...
        <section id="projetos" className="section-wrap projects-section">
          ...
          <div className="projects-grid">{projects.map((project) => <a key={project.title} href={project.link} target="_blank" rel="noreferrer" className={`project-card project-card--${project.accent}`}><div className="project-card__header"><span>{project.number}</span><span>abrir projeto ↗</span></div><div><p className="project-card__type">{project.type}</p><h3>{project.title}</h3><p className="project-card__description">{project.description}</p></div><ul>{project.tech.map((item) => <li key={item}>{item}</li>)}</ul></a>)}</div>
        </section>

        <section className="section-wrap education"><div className="education__label"><span>04</span><p>JORNADA<br />ACADÊMICA</p></div><div className="education__items"><article><span className="education__date">2024 — 2026</span><h3>Ensino Médio + Técnico em Informática</h3><p>Colégios Univap · Unidade Centro</p></article><article><span className="education__date">2026</span><h3>Estágio em sistemas web</h3><p>Colégio Univap · desenvolvimento e manutenção de sistemas para a escola</p></article><article><span className="education__date">EXTRAS</span><h3>Python, inglês para TI, hackathons e feiras tech</h3><p>Aprendizado contínuo dentro e fora da sala de aula.</p></article></div></section>
